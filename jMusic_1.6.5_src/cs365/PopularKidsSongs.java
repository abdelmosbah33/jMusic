...
public class PopularKidsSongs implements JMC{
	...
	public static void main(String[] args) {
		PopularKidsSongs row = new PopularKidsSongs();
		row.makeMusicData();
		row.structureData();
		row.playAndSave();
	}
	
	public PopularKidsSongs() {
		score = new Score("Row Your Boat", 120.0);
		/*
			flute = new Part(VIOLIN,0);
			trumpet = new Part(CHURCH_ORGAN,1);
			clarinet = new Part(SITAR,2);
		*/
		flute = new Part(FANTASIA,0);
		trumpet = new Part(TROMBONE,1);
		clarinet = new Part(APPLAUSE,2);
